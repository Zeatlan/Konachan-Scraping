...
const scraping = async (puppet: KonachanPuppet, config: IConfig): Promise<unknown> => {
  console.log('################################')
  ...
  const downloader = new Downloader(imagesUrl, artistName, config);
  ...
  promises = [];
  nbImages = [];
  return Promise.resolve(scraping(puppet, config));
}
